...
public class Obstacle {
    ...
    private ArrayList<Bitmap> obstacles_broken;
    private Bitmap imgObstacle_broken;
     ...
     */
    public Obstacle(Context context, int posXObstacle, int posY, int speed, int screenWidth, int screenHeight, ArrayList<Bitmap> obstacles,ArrayList<Bitmap> obstacles_broken) {
        ...
        this.obstacles_broken=obstacles_broken;
        i=(int) (Math.random() * obstacles.size());
        this.imgObstacle = this.obstacles.get(i);
        this.imgObstacle_broken=this.obstacles_broken.get(i);
    ...
    public void dibujar(Canvas c) {
        if(broken){
            c.drawBitmap(imgObstacle_broken, posX, posY, pObst);
        }
        else {
    ...
    public void mover() {
        ...
        if (System.currentTimeMillis() - tiempoActual > drawTime) {
            ...
            if (this.posX + imgObstacle.getWidth() < 0) {
                int i=(int) (Math.random() * obstacles.size());
                this.imgObstacle = this.obstacles.get(i);
                this.imgObstacle_broken=this.obstacles_broken.get(i);
                this.broken=false;
